Build a fullscreen hero landing page for a creative agency called "Ninjutsu Creative" using Html, Tailwind CSS. The page should be a single viewport-height section with a looping background video and all content overlaid on top.
...
- Left: brand name "Ninjutsu Creative" in `font-podium`, white, bold, uppercase, `text-2xl sm:text-3xl`, `tracking-wider`.
...
3. **Subtext:** "We design, disrupt, and conquer" (line break) " to ensure your brand doesn't just compete --" then bold white "it dominates.." in `text-white/70`, `text-sm sm:text-base`, `font-inter`, `leading-relaxed`, `max-w-md`. Uses `animate-fade-up-delay-2` (0.4s delay). `mt-6 lg:mt-8`.
...
Make everything fully mobile responsive. Use a single `index.html` component with `useState` for the menu toggle. No routing needed.

After that, complete the other sections that are listed in the header. Use the same font style and color as the hero section.